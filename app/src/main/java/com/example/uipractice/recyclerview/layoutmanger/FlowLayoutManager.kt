...
class FlowLayoutManager: RecyclerView.LayoutManager() {
    private var mVerticalOffset //竖直偏移量 每次换行时，要根据这个offset判断
            = 0
    private var mFirstVisiPos //屏幕可见的第一个View的Position
            = 0
    private var mLastVisiPos //屏幕可见的最后一个View的Position
            = 0

    private var mItemRects //key 是View的position，保存View的bounds 和 显示标志，
            : SparseArray<Rect>? = null

    init {
        isAutoMeasureEnabled = true
        mItemRects = SparseArray<Rect>()
    }
    ...
    private fun fill(recycler: Recycler, state: RecyclerView.State, dy: Int): Int {
        ...
        if (dy >= 0) {
            ...
            val lastChild: View? = getChildAt(childCount - 1)
